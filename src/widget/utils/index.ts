import { BranchType, ProjectType } from "widget/api/index";
import { SelectOptionType } from "widget/components/Select";
import { HTMLMethodsType } from "widget/html/index";
import { HTMLString, PrefixerType } from "widget/types";

export const utils = (c: PrefixerType, htmlMethods: HTMLMethodsType) => {

  const setFormDisabled = (disabled: boolean) => {
    const form = document.querySelector(`.${c('form')}`);
    if (!form) return;
    form.querySelectorAll('input,button').forEach((item: Element) => {
      (item as HTMLInputElement).disabled = disabled;
    });
    form.classList[disabled ? 'add' : 'remove'](c('form-disabled'))
  };

  const clearForm = (cardId: string) => {
    const form = document.querySelector(`.${c('form')}`);
    if (!form) return;
  ...
  const setLoading = (element: Element | null, loading: boolean, disableForm: boolean = true) => {
    if (!element) return;
  ...
  const setErrorMessages = (arr?: string[], isModal = true) => {
  ...
  const branchTypeArr: SelectOptionType[] = [
    {
      id: 'feature',
      name: 'Feature',
      default: true
  ...
  function formatMRNumber(count: number) {
    const d = count % 10;
    switch (true) {
      case d === 0:
  ...
  function formatBranchNumber(count: number) {
    const d = count % 10;
    switch (true) {
      case d === 0:
        return `${count} веток`;
  ...
  function parseGitlabUrls(
    projectData: Record<string, { mrIds: Set<string>; branches: Set<string> }>,
    ...
    if (result) {
      ...
      if (linkType === 'compare') {
        /** допускаем что никто не включит в название ветки "..." */
